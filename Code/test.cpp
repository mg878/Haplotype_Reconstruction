...
//Given a timepoint (i.e 0 for before and 1 for after the transmission), 
//this function finds the likelihood of a set of haplotypes with their contribution
...
{
	//has to go three columns back from the last column to get the first timepoint 
	if (timepoint == 0)
	{
		timepoint = 3;
	}
	//the last column corresponds to the last timepoint
	else if (timepoint == 1)
	{
		timepoint = 1;
	}
	...
	for (unsigned int i = 0; i<CONTRIBS.size(); i++)
	{
		...
		ULTIMA_THULE += Likelihood(660.774, nn, inf);
		temp = q;
	}
	return ULTIMA_THULE;
}
