...
double Dirichlet_m(double timepoint, vector<string> haplotypes, vector<double> freq, vector<vector<vector<int>>> &CONTRIBS, vector<vector<vector<string>>> &partitions)
{
	...
	for (unsigned int i = 0; i<CONTRIBS.size(); i++)
	{
		...
		ULTIMA_THULE += Likelihood(1000000, nn, inf);
		temp = q;
	}
	return ULTIMA_THULE;
}
...
int main(int argc, char* argv[])
{
	...
	for (unsigned int i = 0; i < rows.size(); i++)
	{
		...
		for (unsigned int j = 1; j < sz1 - 3; j++)
		{
			int count = -1;
	...
	while (BIC_check > 0)
	{
		...
		for (unsigned int sample = 0; sample < 5; sample++)
		{
			...
			double THRESHOLD = 1.5 * loop * read_count;
			double attempts = 1;
			...
			for (unsigned int jjj = 0; jjj < 150; jjj++)
			{
				...
				for (unsigned int i = 0; i < loci_numberX; i++)
				{
					...
					replaceX.push_back(shuffle[randomX][shuf_sizeX - 3][i]);
			...
			while (true)
			{
				...
				for (unsigned int i = 0; i < loci_number; i++)
				{
					...
					REPLACE.push_back(shuffle[random][shuf_size - 3][i]);
				...
				L_store = OptimumFreqs(1, candidates, init_freqs1, CONTRIBS, partitions);
		...
		{
			cout << temp_candidates[max_it][i] << '\t' << freqs_1[max_it][i] << endl;
		}
		...
		if (BIC_check>0)
		{
			...
			for (unsigned int i = 0; i < temp_candidates[max_it].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[max_it][i] << "\t" << freqs_1[max_it][i] << endl;
			}
			myfile.close();
