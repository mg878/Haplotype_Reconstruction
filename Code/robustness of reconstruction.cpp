...
double Dirichlet_m(double timepoint, vector<string> haplotypes, vector<double> freq, vector<vector<vector<int>>> &CONTRIBS, vector<vector<vector<string>>> &partitions)
{
	...
	for (unsigned int i = 0; i<CONTRIBS.size(); i++)
	{
		...
		ULTIMA_THULE += Likelihood(211.958, nn, inf);
		temp = q;
	}
	return ULTIMA_THULE;
}
...
int main(int argc, char* argv[])
{
	...
	while (BIC_check > 0)
	{
		...
		if (BIC_check>0)
		{
			...
			ofstream myfile;
			myfile.open("outcome_0.txt");
			for (unsigned int i = 0; i < temp_candidates[0].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[0][i] << "\t" << freqs_1[0][i] << "\t" << freqs_2[0][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_1.txt");
			for (unsigned int i = 0; i < temp_candidates[1].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[1][i] << "\t" << freqs_1[1][i] << "\t" << freqs_2[1][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_2.txt");
			for (unsigned int i = 0; i < temp_candidates[2].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[2][i] << "\t" << freqs_1[2][i] << "\t" << freqs_2[2][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_3.txt");
			for (unsigned int i = 0; i < temp_candidates[3].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[3][i] << "\t" << freqs_1[3][i] << "\t" << freqs_2[3][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_4.txt");
			for (unsigned int i = 0; i < temp_candidates[4].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[4][i] << "\t" << freqs_1[4][i] << "\t" << freqs_2[4][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_5.txt");
			for (unsigned int i = 0; i < temp_candidates[5].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[5][i] << "\t" << freqs_1[5][i] << "\t" << freqs_2[5][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_6.txt");
			for (unsigned int i = 0; i < temp_candidates[6].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[6][i] << "\t" << freqs_1[6][i] << "\t" << freqs_2[6][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_7.txt");
			for (unsigned int i = 0; i < temp_candidates[7].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[7][i] << "\t" << freqs_1[7][i] << "\t" << freqs_2[7][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_8.txt");
			for (unsigned int i = 0; i < temp_candidates[8].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[8][i] << "\t" << freqs_1[8][i] << "\t" << freqs_2[8][i] << endl;
			}
			myfile.close();

			ofstream myfile;
			myfile.open("outcome_9.txt");
			for (unsigned int i = 0; i < temp_candidates[9].size(); i++)
			{
				myfile << i + 1 << "\t" << temp_candidates[9][i] << "\t" << freqs_1[9][i] << "\t" << freqs_2[9][i] << endl;
			}
			myfile.close();
